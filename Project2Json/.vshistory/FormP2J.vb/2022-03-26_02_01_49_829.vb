...
Public Class FormP2J
    ...
    Private Sub CreateJson_Click(sender As Object, e As EventArgs) Handles CreateJson.Click
        ...
        ' Check for G:
        Using myLogFile As IO.StreamWriter = My.Computer.FileSystem.OpenTextFileWriter(mySavePath & "\" & myFileName & ".log", False, utf8withoutBOM)
            Try
                ...
                Using myDataTable As New DataTable
                    ...
                    Using mySqlConnection As New SqlConnection("Data Source=RKAMVGEMSDBP\TEAMMATE;Database=TeamMate_PROD;Integrated Security=True")
                        ...
                        Using mySQLDataAdapter As New SqlDataAdapter(mySql, mySqlConnection)
                            myLogFile.WriteLine("Start Fill myDataTable")
                        ...
                        If myDataTable.Rows Is Nothing Then
                            ...
                            Exit Sub
                        End If
                        For i = 0 To myDataTable.Rows(1).ItemArray.Length - 1
                            myLogFile.WriteLine("#" & i & "#" & myDataTable.Rows(1).Item(i).GetType.ToString())
                        Next
                        ...
                        mySql = "Select 2 As X_Sequence, dbo.NG_Project.ID As P_ID, dbo.NG_Project.Title As P_Title, '-' As P_Code, '-' As P_Grade, '-' As P_Part, '-' As P_Part_Text, '-' As P_Sort, dbo.NG_Folder.Title As S_Title, '-' As F_Number, '-' As F_Title, '-' As F_Rating, '-' As F_Text, '-' As A_Number, '-' As A_Text, '-' As A_Responsible, {d'19000101'} As A_DueDate "
                        ...
                        For Each myDataRow As DataRow In mySortedTable.Rows
                            ...
                            For i As Integer = 0 To myDataRow.ItemArray.Length - 1
                                ...
                                If Not IsDBNull(myDataRow.Item(i)) Then
                                    myLog = myLog & i & myFieldName & "#" & myDataRow.Item(i).GetType().ToString()
                                    If InStr(myDataRow.Item(i).GetType().ToString(), "DateTime") <> 0 Then
                                        ...
                                        ' This here always produces MM/DD/YYYY, i.e. US format no matter what the setting
                                        myDate = CType(myDataRow.Item(i), DateTime)
                                        myText = "converted:" + myDate.ToString("MM/dd/yyyy", CultureInfo.InvariantCulture)
                                    Else
                                ...
                                ' with/without delimiter depending on whether first entry in row or not
                                If myRow = "" Then
                                    If myText <> "" Then
